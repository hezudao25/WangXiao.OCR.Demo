...
$.fn.Huploadify = function(opts){
	...
	var defaults = {
        ...
        onUploadAllComplete: null,//所有上传完成的动作
	  ...
	  var fileObj = {
          ...
          onSelect: function (files) {
              ...
              if (files.length > option.queueSizeLimit) {
                  alert('上传文件个数不能超过' + option.queueSizeLimit + '！');
                  return;
              }
				for(var i=0,len=files.length;i<len;i++){
					var file = files[i];
              ...
              option.onUploadAllComplete && option.onUploadAllComplete(files);
